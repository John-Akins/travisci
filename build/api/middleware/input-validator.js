...
inputValidator.signin = function () {
  return [body("email", "please enter a valid email").not().isEmpty().isEmail(), body("password", "minimum of 8 characters required").not().isEmpty().isLength({
    min: 8
  }), sanitizeBody("email").trim().escape(), sanitizeBody("password").trim().escape()];
};

inputValidator.createUser = function () {
  return [body("firstName").not().isEmpty().isLength({
    min: 2
